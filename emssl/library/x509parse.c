                          ...
                          x509_name *cur )
{
    ...
    do
    {
        ...
        if( *p != end )
        {
            use->next = (x509_name *) X_MALLOC(NULL,
                    sizeof( x509_name ) );
    ...
    cur->next = (x509_name *) X_MALLOC(NULL,
         sizeof( x509_name ) );
                                      ...
                                      x509_sequence *subject_alt_name )
{
    ...
    while( *p < end )
    {
        ...
        *p += buf->len;
        ...
        if (*p < end)
        {
            cur->next = (asn1_sequence *) X_MALLOC(NULL,
                             ...
                             x509_crl_entry *entry )
{
    ...
    while( *p < end )
    {
        ...
        if ( *p < end )
        {
            cur_entry->next = X_MALLOC(NULL, sizeof( x509_crl_entry ) );
...
int x509parse_crt_der( x509_cert *crt, const unsigned char *buf, size_t buflen )
{
    ...
    p = (unsigned char *) X_MALLOC(NULL, len = buflen );
...
int x509parse_crt( x509_cert *chain, const unsigned char *buf, size_t buflen )
{
    ...
    {
        crt->next = (x509_cert *) X_MALLOC(NULL, sizeof( x509_cert ) );
...
int x509parse_crl( x509_crl *chain, const unsigned char *buf, size_t buflen )
{
    ...
    {
        crl->next = (x509_crl *) X_MALLOC(NULL, sizeof( x509_crl ) );
...
int load_file( const char *path, unsigned char **buf, size_t *n )
{
    ...
    *n = (size_t) ftell( f );
    ...
    if( ( *buf = (unsigned char *) X_MALLOC(NULL, *n + 1 ) ) == NULL )
        return( POLARSSL_ERR_X509_MALLOC_FAILED );
...
void x509_crl_free( x509_crl *crl )
{
    ...
    do
    {
        ...
        while( entry_cur != NULL )
        {
            ...
            X_FREE(NULL, entry_prv );
        }

        if( crl_cur->raw.p != NULL )
        {
            memset( crl_cur->raw.p, 0, crl_cur->raw.len );
            X_FREE(NULL, crl_cur->raw.p );
        }
