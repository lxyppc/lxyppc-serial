...
static int ssl_compress_buf( ssl_context *ssl )
{
    ...
    msg_pre = (unsigned char*) X_MALLOC(NULL, len_pre );
    if( msg_pre == NULL )
    {
...
int ssl_parse_certificate( ssl_context *ssl )
{
    ...
    if( ( ssl->session_negotiate->peer_cert = (x509_cert *) X_MALLOC(NULL,
...
int ssl_handshake_init( ssl_context *ssl )
{
    if( ssl->transform_negotiate )
        ...
        ssl->transform_negotiate = X_MALLOC(NULL, sizeof(ssl_transform) );

    if( ssl->session_negotiate )
        ssl_session_free( ssl->session_negotiate );
    else
        ssl->session_negotiate = X_MALLOC(NULL, sizeof(ssl_session) );

    if( ssl->handshake )
        ssl_handshake_free( ssl->handshake );
    else
        ssl->handshake = X_MALLOC(NULL, sizeof(ssl_handshake_params) );
...
int ssl_init( ssl_context *ssl )
{
     ...
     */
    ssl->in_ctr = (unsigned char *) X_MALLOC(NULL, len );
    ...
    ssl->out_ctr = (unsigned char *) X_MALLOC(NULL, len );
...
int ssl_set_hostname( ssl_context *ssl, const char *hostname )
{
    ...
    ssl->hostname = (unsigned char *) X_MALLOC(NULL, ssl->hostname_len + 1 );
...
void ssl_free( ssl_context *ssl )
{
    ...
    if( ssl->handshake )
    {
        ...
        X_FREE(NULL, ssl->handshake );
        X_FREE(NULL, ssl->transform_negotiate );
        X_FREE(NULL, ssl->session_negotiate );
    }

    if ( ssl->hostname != NULL)
    {
        memset( ssl->hostname, 0, ssl->hostname_len );
        X_FREE(NULL, ssl->hostname );
        ssl->hostname_len = 0;
    }
