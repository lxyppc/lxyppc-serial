...
/* simple add a gl viewer to the xtoolbox
...
bool lqobject3ds_load1(Object3ds* w, const char* o_name, const char* tname)
{
    char obj_name[256];
    char t_name[256];
    strcpy(obj_name, o_name);
    strcpy(t_name, tname);
    return w->ObjLoad(obj_name,t_name) != 0;
}
bool lqobject3ds_load2(Object3ds* w, const char* o_name)
{
    return lqobject3ds_load1(w,o_name,"");
}
int lqobject3ds_polygons_qty(Object3ds* w) { return w->GetInfo(1); }
int lqobject3ds_vertices_qty(Object3ds* w) { return w->GetInfo(2); }
int lqobject3ds_size(Object3ds* w) { return w->GetInfo(3); }
void lqobject3ds_render1(Object3ds* w, const QList<double>& p, float scale, int shadeModel, int model)
{
    if(p.count()<3) return;
    float pos[3];
    pos[0] = (float)p[0];
    pos[1] = (float)p[1];
    pos[2] = (float)p[2];
    w->render(pos,scale,shadeModel,model,0);
}

void lqobject3ds_render2(Object3ds* w, const QList<double>& p, float scale, int shadeModel)
{
    lqobject3ds_render1(w,p,scale,shadeModel,0);
}

void lqobject3ds_render3(Object3ds* w, const QList<double>& p, float scale)
{
    lqobject3ds_render1(w,p,scale,0,0);
}

void lqobject3ds_render4(Object3ds* w, const QList<double>& p)
{
    lqobject3ds_render1(w,p,1.0,0,0);
}

void lqobject3ds_render5(Object3ds* w)
{
    float p[3] = {0.0,0.0,0.0};
    w->render(p,1.0,0,0,0);
}

void lqobject3ds_render6(Object3ds* w, float scale)
{
    (void)self;
    float p[3] = {0.0,0.0,0.0};
    w->render(p,scale,0,0,0);
}

LQObject3DS lqobject3ds()
{
    return
    myclass_<Object3ds>("QObject3ds")
    .def(constructor<>())
    .def("load", lqobject3ds_load1)
    .def("load", lqobject3ds_load2)
    .def_readonly("hasTexture", &Object3ds::HasTexture)
    .property("name", &Object3ds::GetName)
    .property("polygonsQty", lqobject3ds_polygons_qty)
    .property("verticesQty", lqobject3ds_vertices_qty)
    .property("size", lqobject3ds_size)
    .def("getInfo", &Object3ds::GetInfo)
    .def("render", lqobject3ds_render1)
    .def("render", lqobject3ds_render2)
    .def("render", lqobject3ds_render3)
    .def("render", lqobject3ds_render4)
    .def("render", lqobject3ds_render5)
    .def("render", lqobject3ds_render6)
    ;
}
