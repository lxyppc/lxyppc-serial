/****************************************************************************
** Meta object code from reading C++ file 'frame.h'
**
** Created: Tue Aug 28 15:46:00 2012
**      by: The Qt Meta Object Compiler version 62 (Qt 4.6.2)
**
** WARNING! All changes made in this file will be lost!
*****************************************************************************/

#include "../frame.h"
#if !defined(Q_MOC_OUTPUT_REVISION)
#error "The header file 'frame.h' doesn't include <QObject>."
#elif Q_MOC_OUTPUT_REVISION != 62
#error "This file was generated using the moc from 4.6.2. It"
#error "cannot be used with the include files from this version of Qt."
#error "(The moc has changed too much.)"
#endif

QT_BEGIN_MOC_NAMESPACE
static const uint qt_meta_data_qglviewer__Frame[] = {

 // content:
       4,       // revision
       0,       // classname
       0,    0, // classinfo
       3,   14, // methods
       0,    0, // properties
       0,    0, // enums/sets
       0,    0, // constructors
       0,       // flags
       2,       // signalCount

 // signals: signature, parameters, type, tag, flags
      18,   17,   17,   17, 0x05,
      29,   17,   17,   17, 0x05,

 // slots: signature, parameters, type, tag, flags
      52,   44,   17,   17, 0x0a,

       0        // eod
};

static const char qt_meta_stringdata_qglviewer__Frame[] = {
    "qglviewer::Frame\0\0modified()\0"
    "interpolated()\0element\0"
    "initFromDOMElement(QDomElement)\0"
};

const QMetaObject qglviewer::Frame::staticMetaObject = {
    { &QObject::staticMetaObject, qt_meta_stringdata_qglviewer__Frame,
      qt_meta_data_qglviewer__Frame, 0 }
};

#ifdef Q_NO_DATA_RELOCATION
const QMetaObject &qglviewer::Frame::getStaticMetaObject() { return staticMetaObject; }
#endif //Q_NO_DATA_RELOCATION

const QMetaObject *qglviewer::Frame::metaObject() const
{
    return QObject::d_ptr->metaObject ? QObject::d_ptr->metaObject : &staticMetaObject;
}

void *qglviewer::Frame::qt_metacast(const char *_clname)
{
    if (!_clname) return 0;
    if (!strcmp(_clname, qt_meta_stringdata_qglviewer__Frame))
        return static_cast<void*>(const_cast< Frame*>(this));
    return QObject::qt_metacast(_clname);
}

int qglviewer::Frame::qt_metacall(QMetaObject::Call _c, int _id, void **_a)
{
    _id = QObject::qt_metacall(_c, _id, _a);
    if (_id < 0)
        return _id;
    if (_c == QMetaObject::InvokeMetaMethod) {
        switch (_id) {
        case 0: modified(); break;
        case 1: interpolated(); break;
        case 2: initFromDOMElement((*reinterpret_cast< const QDomElement(*)>(_a[1]))); break;
        default: ;
        }
        _id -= 3;
    }
    return _id;
}

// SIGNAL 0
void qglviewer::Frame::modified()
{
    QMetaObject::activate(this, &staticMetaObject, 0, 0);
}

// SIGNAL 1
void qglviewer::Frame::interpolated()
{
    QMetaObject::activate(this, &staticMetaObject, 1, 0);
}
QT_END_MOC_NAMESPACE
